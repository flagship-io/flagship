...
func HTTPListUsers() ([]models.User, error) {
	return HTTPGetAllPages[models.User](utils.Host + "/v1/accounts/" + viper.GetViper().GetString("account_id") + "/account_environments/" + viper.GetViper().GetString("account_environment_id") + "/users")
}

func HTTPBatchUpdateUsers(data string) ([]byte, error) {
	return HTTPRequest(http.MethodPut, utils.Host+"/v1/accounts/"+viper.GetViper().GetString("account_id")+"/account_environments/"+viper.GetViper().GetString("account_environment_id")+"/users", []byte(data))
}
